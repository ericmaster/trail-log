...
## Content Management
- **Text vs Content**:
  - Use JSON files (`src/lib/i18n/*.json`) for UI labels, buttons, and short text.
  - Use Markdown files (`static/content/{locale}/*.md`) for long-form content (e.g., protocols, policies, guides).
- **Adding New Content**:
  1. Create markdown files in `static/content/en/` and `static/content/es/`.
  2. Use `loadMarkdownContent` from `$lib/markdown` to load them in Svelte components.
  3. Render using `{@html content}` with the `.markdown-content` class for styling.
- **Styling**:
  - Markdown content should be wrapped in a container class (e.g., `.markdown-content`).
  - Use global styles in the component to style headers, lists, and tables generated by the markdown parser.

## Coding Standards & Patterns
- **API**: Ensure modifying `models.py`, `schemas.py`, and the router (e.g., `routers/uploads.py`) in sync when adding new fields.
- **Frontend**:
  - Use `svelte-i18n` for all UI text (`$t("key")`).
  ...
  - **CRITICAL**: For long content pages, ALWAYS use the markdown content pattern. Do NOT put large blocks of text in JSON files or hardcode them in components.
