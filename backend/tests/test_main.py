...
class TestFileUpload:
    ...
    def test_upload_sanitizes_path_traversal_filename(self, tmp_path, monkeypatch):
        ...
        response = client.post(
            "/api/upload/",
            headers=headers,
            files={"file": ("../../evil.fit", b"malicious content", "application/octet-stream")},
        )
        assert response.status_code == 201
        data = response.json()

        saved_path = os.path.abspath(data["filepath"])
        upload_root = os.path.abspath(str(tmp_path))
        assert os.path.commonpath([saved_path, upload_root]) == upload_root
