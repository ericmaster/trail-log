...
class TestFileUpload:
    ...
    def test_upload_sanitizes_path_traversal_filename(self, tmp_path, monkeypatch):
        ...
        response = client.post(
            "/api/upload/",
            headers=headers,
            files={"file": ("../../../../etc/evil.fit", b"malicious content", "application/octet-stream")},
        )
        assert response.status_code == 201
        data = response.json()
        saved_path = Path(data["filepath"]).resolve()
        # The saved file must stay inside the user's upload directory, not
        # escape it via path traversal sequences in the filename.
        assert saved_path.is_relative_to(tmp_path.resolve())
        assert saved_path.name.endswith("_evil.fit")
